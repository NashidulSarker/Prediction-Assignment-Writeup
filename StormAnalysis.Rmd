...
testing.set$classe <- factor(testing.set$classe, levels = levels(pred.tree))
pred.tree <- factor(pred.tree, levels = levels(testing.set$classe))
...
testing.set$classe <- as.factor(testing.set$classe)

# Prediction with Random Forest
# Build model
set.seed(12345)
rf.fit <- randomForest(
  classe ~ .,  # Predict 'classe' using all other columns
  data = training.set,
  ntree = 250
)
...
pred2 <- predict(rf.fit, testing.set[,-ncol(testing.set)])

# Align factor levels for confusionMatrix
testing.set$classe <- factor(testing.set$classe, levels = levels(pred2))
pred2 <- factor(pred2, levels = levels(testing.set$classe))
